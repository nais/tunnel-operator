...
// STUNBind reuses an existing UDP socket for WireGuard traffic.
type STUNBind struct {
	conn      *net.UDPConn
	closeOnce sync.Once
}

// StdNetEndpoint wraps a UDP address as a WireGuard endpoint.
type StdNetEndpoint net.UDPAddr

// NewSTUNBind creates a WireGuard bind backed by an existing UDP socket.
func NewSTUNBind(udpConn *net.UDPConn) *STUNBind {
	return &STUNBind{conn: udpConn}
}

// Open returns a single receive function backed by the wrapped UDP connection.
...
// Close closes the wrapped UDP connection.
...
// Send writes packets to the destination endpoint using the wrapped UDP socket.
...
// ParseEndpoint resolves a host:port string into a WireGuard endpoint.
...
// BatchSize returns the number of packets processed per receive call.
